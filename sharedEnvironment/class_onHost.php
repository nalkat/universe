...
class OnHost {
	...
	private string $userName;		// the username to use for the connections (defaults to $LOGNAME from the environment)
	private string $nodeFile;		// text file containing: id:address[:port]
	private array $nodeList;		// array containing hosts to operate on
	private string $onlyNodes;		// only operate on specified nodes in the list
	private string $identityFile;	// ~$userName/.ssh/id_<type> || /path/to/identity_file
	private bool $parallelExec;	// do parallel execution?
	...
	public function __construct (string $hostList, string $command, ?string $onlyNodes = null, ?bool $parallel = false, ?string $identityFile = null, ?Logger $log = null)
	{
		$this->initializeObject();

		if (($log !== null) || (!is_a($log,"Logger")))
		{
			$logDir = $_SERVER['ENV_LOGDIR'] . "/onHost";
...
//			}
	...
	public function determineNodeWeight (string $node = null) : int
	{
		...
		for ($i = 0; $i < 25; $i++)
		{
			...
			if (@socket_connect($qs,$this->nodeList[$key]['address'],80) === false) break;
			$ts = microtime(true);
	...
	public function isAlive (string $node = null) : bool
	{
		...
		if (@socket_connect($qs, $this->nodeList[$key]['address'], 80) === false)
		{
			return false;				
		}
		$ts = microtime(true);
...
//					}
	...
	private function runCommand (string $command = null):bool
	{
	if ($command === null) return false;
	$output[$this->node]=trim(shell_exec("$command"));
	if ($output !== null)
		return true;
	else
		return false;
...
*/
	...
	public function run(string $command = null):string
	{
		if ($command === null) return false;
		// just be dangerous for now
		$this->command = $command;

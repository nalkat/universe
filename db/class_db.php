  ...
  private $totalBytesIn;
	...
	// dbHost must be populated if this is hit
		...
		else
		{
			...
			$this->dbPort = $dbPort;
			$this->dbName = $dbName;
			$this->dataSource = "host=$dbHost user=$dbUser password=$dbPass dbname=$dbName port=$dbPort";
			try
			{
				if (!($this->__dbSock = pg_connect ($this->dataSource)))
//				if (!is_resource($this->__dbSock) || !is_object($this->__dbSock))
			...
			{
//				echo $dbFail->getMessage() . PHP_EOL;
//				exit ($dbFail->getCode());
			}
		}
	...
	public function __destruct ()
	{
		...
		switch (strtolower($this->dbType))
		{
			case 'pgsql':
				...
				if (is_resource ($this->__dbSock))
				{
					pg_close ($this->__dbSock);
				}
				//transfer this back to the global, even though it should be the same
...
//				exit(1);
	...
	/**
   * Escapes a string to be safely used as a PostgreSQL literal in an SQL query.
   * It also adds the surrounding single quotes.
   *
   * @param string $value The string to escape.
   * @return string|false The escaped and quoted string, or false on error (e.g., not connected).
   */
  ...
  /**
   * Escapes a string content to be safely used within an SQL query for PostgreSQL.
   * Note: This ONLY escapes the content. You'll need to add quotes around it in the SQL.
   * escapeLiteral() is generally safer and preferred for full string literals.
   *
   * @param string $value The string to escape.
...
/*
	public function query(): bool
	{
		...
		if ($this->is_connected())
		{
			if (!empty($this->sqlstr))
			{
				// Check if the query is a prepared statement AND the database type is PostgreSQL
				if (isset($this->prepared) && $this->prepared && strtolower($this->dbType) === 'pgsql')
				{
				  try 
					{
				    $this->EnVision->db_queries++;
	          ...
	          $this->result_set = pg_execute($this->__dbSock, "", $this->params);
	          ...
	          return false;
				...
				{
	        // ******** BEGIN existing code for regular queries and SQLite ********
	        $resultBytes = 0;
					switch (strtolower($this->dbType)) 
					{
	          case 'pgsql':
	            ...
	            if (!is_resource($this->result_set))
	              ...
	              $this->EnVision->db_failed_queries++;
					...
					// ******** END existing code for regular queries and SQLite ********
				}
	    }
			else
			{
...
// CHANGE LOG
/////////////
// 04/01/2003 Created initial version of the library.
/////////////
// 04/01/2003 Added private class variables for user,pass,host,port,datasource and the socket connection.
// 04/01/2003 Added public class variables for the sql string and result set.
// 04/01/2003 Added code to construct and destroy the object.
// 04/01/2003 Added code to query the database and to determine if it is ready to accept queries.
/////////////
// 06/06/2003 Changed __destruct to free the result resource upon destruction.
/////////////
...
// 05/09/2018 Created 'row' method to return an associative array containing only the values of a single result of a query (must be iterated for all results)
/////////////

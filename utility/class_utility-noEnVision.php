...
//	public  static $Telemetry = null;
	...
	{
		if ((!is_resource(self::$socket)) || ((strcmp(strtolower(get_resource_type(self::$socket)), "socket")) !== 0)) return false;
			return true;
	}
	...
	{
		if ((!is_resource($socket)) || ((strcmp(strtolower(get_resource_type($socket)), "socket")) !== 0)) return false;
		self::$socket =& $socket;
	...
	{
		if (!is_a($log,"Logger")) return false;
		$ret = false;
		if ($oFlag & L_ACCESS)
		{
			self::$aLog =& $log;
	...
	{
		if (is_a($resource, "Logger")) return true; else return false;
	}
	...
	public static function write (string $what = null, int $logLevel = null, int $oFlags = null)
	{
		try 
		{
			...
			else
			{
				if ($oFlags & L_ACCESS)
				{
					if (!self::isLog(self::$aLog)) throw new Exception ("Attempting to write log data to nonLog objects", 1);
					if (!self::is_connected()) throw new Exception ("Attempting to write disconnected socket", 2);
					$ret = self::$aLog->writeLog($what, $logLevel);
				}
				if ($oFlags & L_CONSOLE)
				{
					if (!self::isLog(self::$cLog)) throw new Exception ("Attempting to write log data to nonLog objects", 1);
					$ret = self::$cLog->writeLog($what, $logLevel);
				}
				if ($oFlags & L_DEBUG)
				{
					if (!self::isLog(self::$dlog)) throw new Exception ("Attempting to write log data to nonLog objects", 1);
					$ret = self::$dLog->debug($what, $logLevel);
				}
				if ($oFlags & L_ERROR)
				{
					if (!self::isLog(self::$eLog)) throw new Exception ("Attempting to write log data to nonLog objects", 1);
					$ret = self::$eLog->writeLog($what, $logLevel);
				}
			}
	...
	public static function query_alnumu (string $query) : string
	{
		...
		if (preg_match('/[^a-zA-Z_]/', $query)) return "";
		else return "$query";
	}
	...
	public static function query_alnum (string $query) : string
	{
		...
		if (preg_match('/[^a-zA-Z0-9]/', $query)) return "";
		else
		{
			return "$query";
		}
	...
	{
		if (preg_match('/[^0-9\-\.\+,%\$^\(\)]/', $query)) return "";
		else return "$query";
	}
	...
	{
		if (preg_match('/[^a-zA-Z]/', $query)) return "";
		else return "$query";
	}
